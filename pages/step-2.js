...
const defaultRoom = {
  ...
  flooring: { type: '', area: '' },
  tile: {
    hasTubTile: false, hasBacksplash: false, edge: '', tileColor: '',
    edgeColor: '', edgeSize: '', grout: '', groutSealer: false
  },
  painting: {
    ceiling: false, walls: false, baseAndCase: false, cabinets: false, sealerRequired: false
  },
  drywall: {
    ceilingType: '', drywallPatches: false, moldDrywall: false,
    insulation: false, deleteIntercom: false, backingRequired: false
  },
  electrical: {
    outlets: '', switches: '', switchType: '', smokeDetector: false,
    gfci: '', lightFixtures: ''
  },
  cabinets: {
    upperQty: '', lowerQty: '', linearFeet: '', gableEnds: '',
    vanityQty: '', vanitySize: ''
  },
  countertops: { sqft: '', type: '' },
  plumbing: {
    tub: false, tubDirection: '', tubSize: '', showerRod: false,
    toilet: false, sink: false, absFittings: false, copperPipe: false,
    shutOffs: false, absPipe: false, pTrap: false, pTrapCleanout: false,
    plumbingNotes: ''
  }
};
    ...
    <div style={{ padding: '1rem' }}>
      ...
      {rooms.map((room, index) => (
        <div key={index} style={{
          ...
          <input value={room.label} onChange={(e) => handleChange(index, null, 'label', e.target.value)} placeholder="e.g. Main Bath, Kitchen A" style={{ width: '100%' }} />
          ...
          <input value={room.width} placeholder="Width" onChange={(e) => handleChange(index, null, 'width', e.target.value)} />
          ...
          <label>Edge:</label><input value={room.tile.edge} onChange={(e) => handleChange(index, 'tile', 'edge', e.target.value)} />
          <label>Tile Color:</label><input value={room.tile.tileColor} onChange={(e) => handleChange(index, 'tile', 'tileColor', e.target.value)} />
          <label>Edge Color:</label><input value={room.tile.edgeColor} onChange={(e) => handleChange(index, 'tile', 'edgeColor', e.target.value)} />
          <label>Edge Size:</label><input value={room.tile.edgeSize} onChange={(e) => handleChange(index, 'tile', 'edgeSize', e.target.value)} />
          <label>Grout:</label><input value={room.tile.grout} onChange={(e) => handleChange(index, 'tile', 'grout', e.target.value)} />
          ...
          {/* Section: Drywall */}
          <h3>🧱 Drywall</h3>
          <label>Ceiling Type:</label><input value={room.drywall.ceilingType} onChange={(e) => handleChange(index, 'drywall', 'ceilingType', e.target.value)} />
          ...
          {/* Section: Electrical */}
          <h3>💡 Electrical</h3>
          <label>Outlets:</label><input value={room.electrical.outlets} onChange={(e) => handleChange(index, 'electrical', 'outlets', e.target.value)} />
          <label>Switches:</label><input value={room.electrical.switches} onChange={(e) => handleChange(index, 'electrical', 'switches', e.target.value)} />
          <label>Switch Type:</label><input value={room.electrical.switchType} onChange={(e) => handleChange(index, 'electrical', 'switchType', e.target.value)} />
          ...
          <label>GFCI:</label><input value={room.electrical.gfci} onChange={(e) => handleChange(index, 'electrical', 'gfci', e.target.value)} />
          <label>Light Fixtures:</label><input value={room.electrical.lightFixtures} onChange={(e) => handleChange(index, 'electrical', 'lightFixtures', e.target.value)} />

          {/* Section: Cabinets */}
          <h3>🪟 Cabinets</h3>
          <label>Upper Qty:</label><input value={room.cabinets.upperQty} onChange={(e) => handleChange(index, 'cabinets', 'upperQty', e.target.value)} />
          <label>Lower Qty:</label><input value={room.cabinets.lowerQty} onChange={(e) => handleChange(index, 'cabinets', 'lowerQty', e.target.value)} />
          <label>Linear Feet:</label><input value={room.cabinets.linearFeet} onChange={(e) => handleChange(index, 'cabinets', 'linearFeet', e.target.value)} />
          <label>Gable Ends:</label><input value={room.cabinets.gableEnds} onChange={(e) => handleChange(index, 'cabinets', 'gableEnds', e.target.value)} />
          <label>Vanity Qty:</label><input value={room.cabinets.vanityQty} onChange={(e) => handleChange(index, 'cabinets', 'vanityQty', e.target.value)} />
          <label>Vanity Size:</label><input value={room.cabinets.vanitySize} onChange={(e) => handleChange(index, 'cabinets', 'vanitySize', e.target.value)} />

          {/* Section: Countertops */}
          <h3>🧱 Countertops</h3>
          <label>Square Feet:</label><input value={room.countertops.sqft} onChange={(e) => handleChange(index, 'countertops', 'sqft', e.target.value)} />
          <label>Type:</label><input value={room.countertops.type} onChange={(e) => handleChange(index, 'countertops', 'type', e.target.value)} />
          ...
          <label>Direction:</label><input value={room.plumbing.tubDirection} onChange={(e) => handleChange(index, 'plumbing', 'tubDirection', e.target.value)} />
          <label>Size:</label><input value={room.plumbing.tubSize} onChange={(e) => handleChange(index, 'plumbing', 'tubSize', e.target.value)} />
          ...
          <label>Plumbing Notes:</label><textarea value={room.plumbing.plumbingNotes} onChange={(e) => handleChange(index, 'plumbing', 'plumbingNotes', e.target.value)} style={{ width: '100%' }} />
        </div>
      ))}
